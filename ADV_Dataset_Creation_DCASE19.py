...
from LCustomdataset_Train import LAEData_Train # Call Customdataloader to read Training Data
from LCustomdataset_Test import LAEData_Test # Call Customdataloader to read Test Data
...
attack=torchattacks.FGSM(Teacher_Model,eps=1/255)
...
def ADV_Dataset(Data,key):
    ...
    key=np.array(key)
    #Data=Data.cpu()
    with h5py.File('ADV_DCASE_FGSM_S1_Train.hdf5', 'w') as f:
...
SAVE_DIR = 'models'
MODEL_SAVE_PATH = os.path.join("/media/mani/Ph.D./PHD_Journals/AK/Adversarial Attacks Defense Using GAN/Adversarial Attack Datasets/DCASE 2019", 'PAM_DCASE19_CNN.pt') # Define Path to save the model 
Teacher_Model.load_state_dict(torch.load(MODEL_SAVE_PATH))
test_loss, test_acc = evaluate(Teacher_Model, device, train_loader, criterion)
